...
setwd('/storage/home/gzx103/group/projects/vision/RNA_pca')

set.seed(2018)
#signal_matrix = read.table('snapshot20_reproduce_2_16lim/atac_20cell.signal.matrix.no0.txt', header=F)[,c(-1,-2,-3,-4)]
signal_matrix = read.table('rnaTPMall_withcoordinates.0.txt', header=T)[,-c(1:6)]
...
breaks = c(seq(.74,1,length=16))

dist.pear <- function(x) as.dist(1-x)
pdf('hc_heatmap.RNA.pdf')

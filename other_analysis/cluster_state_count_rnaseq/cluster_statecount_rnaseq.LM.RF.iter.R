...
mod1 = Mclust(gene_exp, G = 4)
summary(mod1, parameters = TRUE)
c = mod1$classification
qda_mod = qda(gene_exp, c)
...
used_id_RF = sample(dim(dr0_all)[1], 10000)
...
for (j in cell_num[cell_num==cell_num_test]){
	used_id_ct = seq(1,length(rt$y),12) + (j-1)
	dr0_all_tmp = cbind(rt$x[used_id_ct,], rt$x0[used_id_ct,])
}
#RF_gg = predict(modelRF, dr0_all_tmp)
#qda_gg = predict(qda_mod, gene_exp_tmp)$class
gene_exp_genegroup_test = c(gene_exp_genegroup_test0, get_gene_group(gene_exp_tmp))
#gene_exp_genegroup_test = c(gene_exp_genegroup_test, qda_gg)

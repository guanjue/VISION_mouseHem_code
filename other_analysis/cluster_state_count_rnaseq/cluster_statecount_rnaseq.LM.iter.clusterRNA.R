...
### cluster genes
exp_mclust = Mclust(gene_exp, G = 10)
gene_exp_c = exp_mclust$classification
gene_exp_qda = qda(gene_exp, as.factor(gene_exp_c))
gene_exp_genegroup = predict(gene_exp_qda, gene_exp)$class
...
gene_exp_genegroup_test = predict(gene_exp_qda, gene_exp_genegroup_forbenchmark)$class
...
png(paste('test.LM.RNAcluster.chr', chr_num_test, '.', cell_num_test, '.png', sep=''), width=1500, height=500)
par(mfrow=c(1,3)) 
...
for (gg in c(1:length(table(gene_exp_genegroup_test)))){
print(gg)
...
### plot GG
png(paste('test.GG.RNAcluster.chr', chr_num_test, '.', cell_num_test, '.png', sep=''), width=1500, height=500)
par(mfrow=c(1,3)) 
...
### plot split
png(paste('test.GG.RNAcluster.split.chr', chr_num_test, '.', cell_num_test, '.png', sep=''), width=2000, height=500)
par(mfrow=c(1,4)) 
for (i in c(1:4)){
...
png(paste('test.GG.RNAcluster.split.P.chr', chr_num_test, '.', cell_num_test, '.png', sep=''), width=2000, height=500)
par(mfrow=c(1,4)) 
for (i in c(1:4)){
used_id_gg_plot = gene_exp_genegroup_vec_testing_new==i
...
dev.off()
png(paste('test.GG.RNAcluster.split.D.chr', chr_num_test, '.', cell_num_test, '.png', sep=''), width=2000, height=500)
par(mfrow=c(1,4)) 
for (i in c(1:4)){
used_id_gg_plot = gene_exp_genegroup_vec_testing_new==i
...
write.table(LM_r2, 'LM_r2.RNAcluster.txt', quote=F, col.names=F, row.names=F, sep='\t')
write.table(LM_P_r2, 'LM_P_r2.RNAcluster.txt', quote=F, col.names=F, row.names=F, sep='\t')
write.table(LM_D_r2, 'LM_D_r2.RNAcluster.txt', quote=F, col.names=F, row.names=F, sep='\t')
write.table(GG_r2_benchmark, 'GG_r2_benchmark.RNAcluster.txt', quote=F, col.names=F, row.names=F, sep='\t')
write.table(GG_r2, 'GG_r2.RNAcluster.txt', quote=F, col.names=F, row.names=F, sep='\t')
write.table(GG_P_r2, 'GG_P_r2.RNAcluster.txt', quote=F, col.names=F, row.names=F, sep='\t')
write.table(GG_D_r2, 'GG_D_r2.RNAcluster.txt', quote=F, col.names=F, row.names=F, sep='\t')

png('r2_box.RNAcluster.png')
...
png('test.m_vs_sd.RNAcluster.png', width=1000, height=1000)
par(mfrow=c(2,2))
plot(m, s)
abline(v=-4, col='red')
abline(h=2, col='red')

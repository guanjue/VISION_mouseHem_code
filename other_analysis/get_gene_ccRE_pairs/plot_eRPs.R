...
tss_eRPs = read.table('~/group/projects/vision/rna/tss_eRP_1kb.txt', header=T)
dist_eRPs = read.table('~/group/projects/vision/rna/dist_eRP_1kb.txt', header=T)
...
tss_eRPs_s = apply(tss_eRPs, 2, function(x) scale(x))
rownames(tss_eRPs_s) = c(0:26)

dist_eRPs_s = apply(dist_eRPs, 2, function(x) scale(x))
rownames(dist_eRPs_s) = c(0:26)

all_mat = cbind(tss_eRPs_s, dist_eRPs_s)
all_rowsum = apply(all_mat, 1, function(x) sum(x) )
all_colsum = apply(all_mat, 2, function(x) sum(x) )

dist <- dist(all_mat, method = "euclidean") # distance matrix
fit <- hclust(dist, method="ward.D2") 
hclust_order = fit$order

library(pheatmap)

my_colorbar=colorRampPalette(c('green', 'white', 'red'))(n = 128)

colnames(all_mat) = c('tss1', 'tss2', 'tss3', 'tss4', 'dist1', 'dist2', 'dist3', 'dist4')

pdf('eRPs.1kb.pdf', width=4)
pheatmap(all_mat[order(all_rowsum),], color=my_colorbar, cluster_rows = FALSE, cluster_cols = F,annotation_names_row=FALSE,annotation_names_col=FALSE,show_rownames=T,show_colnames=T)
dev.off()

png('eRPs.1kb.png', width=200)
pheatmap(all_mat[order(all_rowsum),], color=my_colorbar, cluster_rows = FALSE, cluster_cols = F,annotation_names_row=FALSE,annotation_names_col=FALSE,show_rownames=T,show_colnames=T)
dev.off()

pdf('eRPs.1kb.hclust.pdf', width=4)
pheatmap(all_mat[rev(hclust_order),], color=my_colorbar, cluster_rows = FALSE, cluster_cols = F,annotation_names_row=FALSE,annotation_names_col=FALSE,show_rownames=T,show_colnames=T)
dev.off()
...
pdf('pknorm_2_16lim_ref1mo_0424_heatmap.eRP_sort.pdf', width=4)
...
pdf('pknorm_2_16lim_ref1mo_0424_heatmap.eRP_sort.hclust.pdf', width=4)
createHeatmap_sort_eRP('~/group/projects/vision/pknorm_2_16lim_ref1mo_sample1mo_0424/pknorm_2_16lim_ref1mo_0424_lesshet.para0', sortstate=TRUE)
dev.off()

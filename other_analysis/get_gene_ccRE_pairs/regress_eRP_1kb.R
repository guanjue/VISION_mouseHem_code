...
for ( j in chrom_id){
print(j)
Rdat_name = paste('vision_rna_tss1k.chr', toString(j), '.', toString(i), '.1.Rdat', sep='')
load(Rdat_name)
...
write.table(tss_eRP, 'tss_eRP_1kb.txt', sep='\t', quote=F, row.names=T, col.names=T)
write.table(dist_eRP, 'dist_eRP_1kb.txt', sep='\t', quote=F, row.names=T, col.names=T)

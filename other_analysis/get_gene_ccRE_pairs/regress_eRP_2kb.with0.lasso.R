...
###
tss_eRP = cbind(tss_eRP, tss_modellasso$beta[,1])
dist_eRP = cbind(dist_eRP, dist_modellasso$beta[,1])
}
...
write.table(tss_eRP, 'tss_eRP_2kb.with0.lasso.txt', sep='\t', quote=F, row.names=T, col.names=T)
write.table(dist_eRP, 'dist_eRP_2kb.with0.lasso.txt', sep='\t', quote=F, row.names=T, col.names=T)

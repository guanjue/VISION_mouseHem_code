...
################################################################################################
allinfo = read2d_array('vision_rna.gene_ccRE.selected.all.reprod_count120718_withNames.sort.txt', str)
...
for info in allinfo:
	...
	if current_gene == '':
		...
		start_200bp = float_to_str(int(float(info[5]))-200)
		end_200bp = float_to_str(int(float(info[5])))
		if '1' in info[9]:
			ccRE_200bp.write(info[0] + '\t' + start_200bp + '\t' + end_200bp + '\t' + info[7] + '\t' + '1' + '\n')
		else:
			ccRE_200bp.write(info[0] + '\t' + start_200bp + '\t' + end_200bp + '\t' + info[7] + '\t' + '0' + '\n')
		#################
		#################
		...
		call('bedtools map -a tmp.tss.allccREs.bed -b tmp.ccRE_200bp.sort.txt -c 5 -o max > tmp.tss.allccREs.select.bed', shell=True)
		### concatenate to all 

...
head(IDEAS)
IDEAS_state_count = table(IDEAS[,4])
...
### merge TAD level
d12345_info = rbind(d1,d2,d3,d4,d5)
...
### scale
d12345_s = d12345
d12345_s[,-1] = t(apply(d12345[,-1], 1, function(x) x/IDEAS_state_count*mean(IDEAS_state_count) ))
...
cor_dist = cor2dist(histp_cor_mat)
...
dist_within_i = 0
for (j in c(1:i)){
dist_within_j = sum(1-histp_cor_mat[cluster_id==j, cluster_id==j])
...
IDEAS_state_count_merged = c()
for (i in 1:ncluster){
	if (sum(cluster_id_used==i)>1){
	d12345_IDEASps_tmp = rowSums(d12345_IDEASps[,cluster_id_used==i])
	IDEAS_state_count_tmp = sum(IDEAS_state_count[cluster_id_used==i])
	} else{
	d12345_IDEASps_tmp = d12345_IDEASps[,cluster_id_used==i]
	IDEAS_state_count_tmp = IDEAS_state_count[cluster_id_used==i]
	}
	d12345_IDEASps_merged = cbind(d12345_IDEASps_merged, d12345_IDEASps_tmp)
	IDEAS_state_count_merged = c(IDEAS_state_count_merged, IDEAS_state_count_tmp)
}
### scale merge IDEASps
d12345_IDEASps_merged_s = t(apply(d12345_IDEASps_merged, 1, function(x) x/IDEAS_state_count_merged*mean(IDEAS_state_count_merged) ))

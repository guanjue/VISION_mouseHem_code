...
do
echo $i
cat ../output_newid_color.tadonly.bedgraph | awk -F '\t' -v OFS='\t' -v state=$i '{if ($4==state) print $1, $2, $3, $4}' > 'chr19.'$i'.state.qda.bed'
bedtools intersect -a mm10.10KB.chr19.id.bed -b 'chr19.'$i'.state.qda.bed' -wa > 'mm10.10KB.chr19.s'$i'.qda.bed'
done

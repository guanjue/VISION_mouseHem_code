...
d_rc_mat = c()
#1:dim(d0)[1]
plot_region = c(3000:4500)
#plot_region = c(1:6144)
d_rc_lim = d_rc
upperlim = 1000
lowerlim = 0
...
sig = d_rc_lim[i,j]
d_rc_row[j-min(plot_region)+1] = sig
}
d_rc_mat = rbind(d_rc_mat, d_rc_row)
}

colnames(d_rc_mat) = rep('', length(plot_region))
rownames(d_rc_mat) = rep('', length(plot_region))
...
sig = d_rc_lim[bi,bj]
d_rc_mat_ideas_s2[bi-plot_region_min+1, bj-plot_region_min+1] = sig
d_rc_mat_ideas_s2_binary[bi-plot_region_min+1, bj-plot_region_min+1] = 1
}
}
}
...
dist_sd_s2 = c()
dist_n_s2 = c()
for (k in 1:(dim(d_rc_mat_ideas_s2_binary)[1])){
print(k)
sig_dist_vec_tmp = c()
...
#write.table(dist_mean_sd_s2, 'dist_mean_sd_s2.neg.txt', quote=F, sep='\t', col.names=F, row.names=F)



d_rc_mat_ideas_snot2 = d_rc_mat
...
if ((bi-plot_region_min+1>0) & (bj-plot_region_min+1>0) & (bi-plot_region_min+1<plot_region_length) & (bj-plot_region_min+1<plot_region_length)){
sig = d_rc_lim[bi,bj]
d_rc_mat_ideas_snot2[bi-plot_region_min+1, bj-plot_region_min+1] = sig
d_rc_mat_ideas_snot2_binary[bi-plot_region_min+1, bj-plot_region_min+1] = 1
}
}
}
...
dist_mean_sd_CP_s2 = cbind(dist_mean_sd_snot2, dist_mean_sd_s2)

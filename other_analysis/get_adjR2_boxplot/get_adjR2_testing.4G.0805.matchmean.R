...
#y_g = scale(y_g, scale=F)
### get performance
for (l in c(1:12)){
	print(l)
	### train vs test y
	y_g_testing = y_g[,l]
	y_g_training_mean = rowMeans(y_g[,-l])
	...
	### train model
	data_train_all = as.data.frame(cbind(y_g_training, y_g_training_mean, x_g_training, x0_g_training))
	colnames(data_train_all) = c('y',c(1:(2*dim(x_g_training)[2])))
	fit_all = lm(y~., data=data_train_all)
	data_train_tss = as.data.frame(cbind(y_g_training, y_g_training_mean, x0_g_training))
	colnames(data_train_tss) = c('y',c(1:dim(x_g_training)[2]))
	fit_tss = lm(y~., data=data_train_tss)
	data_train_dist = as.data.frame(cbind(y_g_training, y_g_training_mean, x_g_training))
	colnames(data_train_dist) = c('y',c(1:dim(x_g_training)[2]))
	fit_dist = lm(y~., data=data_train_dist)
	### testing
	data_test_all = as.data.frame(cbind(y_g_training_mean, x_g_testing, x0_g_testing))
	...
	data_test_tss = as.data.frame(cbind(y_g_training_mean, x0_g_testing))
	...
	data_test_dist = as.data.frame(cbind(y_g_training_mean, x_g_testing))
	...
	###
	adjR2_mean = adjR2_fun(y_g_testing, y_g_training_mean, 1)
	###
	adjR2_vec_all[l] = adjR2_all
	adjR2_vec_tss[l] = adjR2_tss
	adjR2_vec_dist[l] = adjR2_dist
	adjR2_vec_mean[l] = adjR2_mean
}

pdf(paste('tss_vs_distal.', gg, '.scale.pdf', sep=''), width=3.5, height=3.5)
boxplot(cbind(adjR2_vec_mean, adjR2_vec_tss, adjR2_vec_dist, adjR2_vec_all), col=c('gray', 'red', 'blue', 'orange'), names = c('M', 'P', 'D', 'P & D'), ylim=c(-0.2,1))
dev.off()
print(gg)
}

...
### get performance
adjR2_vec_all_27state = c()
adjR2_vec_tss_27state = c()
adjR2_vec_dist_27state = c()

for (shadow_i in 1:27){
#shadow_i = 1
adjR2_vec_all = c()
adjR2_vec_tss = c()
adjR2_vec_dist = c()
...
for (l in c(1:12)){
	...
	data_test_all[,shadow_i] = sample(as.vector(data_test_all[,shadow_i]))
	data_test_all[,shadow_i+27] = sample(as.vector(data_test_all[,shadow_i+27]))
	...
	data_test_tss[,shadow_i] = sample(as.vector(data_test_tss[,shadow_i]))
	...
	data_test_dist[,shadow_i] = sample(as.vector(data_test_dist[,shadow_i]))
...
adjR2_vec_all_27state = cbind(adjR2_vec_all_27state, adjR2_vec_all)
adjR2_vec_tss_27state = cbind(adjR2_vec_tss_27state, adjR2_vec_tss)
adjR2_vec_dist_27state = cbind(adjR2_vec_dist_27state, adjR2_vec_dist)

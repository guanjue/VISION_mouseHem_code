library(pheatmap)
library(RColorBrewer)

### get tf chip-seq pk total number
tf_files = read.table('../file_list.sub.txt', header=F)
tf_files_names = apply(((tf_files)), 1, function(x) unlist(strsplit(x, '.bb'))[1])
tf_files_names = apply(((tf_files)), 1, function(x) as.character(x))


JI_mat = c()
union_mat = c()
tf_files_names_exist = c()
file_used = c()
for (i in tf_files_names){
i = as.character(i)
print(i)
### input name
input_file_name = paste('JI_matrix_folder_166set/JI_matrix_', i, '.txt', sep='')
### read intersect count of each pk
if(file.exists(input_file_name)){
d = read.table(input_file_name, header=F)
d_JI = (d[,3])
d_union = (d[,2])
JI_mat = rbind(JI_mat, d_JI)
union_mat = rbind(union_mat, d_union)
tf_files_names_exist = c(tf_files_names_exist, i)
file_used = c(file_used, TRUE)
}else{
file_used = c(file_used, FALSE)
}
}

##################
JI_mat = JI_mat[,file_used]
JI_mat = as.matrix(JI_mat)
rownames(JI_mat) = tf_files_names_exist
colnames(JI_mat) = tf_files_names_exist

##################
### 
JI_mat_vector = as.vector(JI_mat)
JI_mat_vector_rmtop = JI_mat_vector[JI_mat_vector<quantile(JI_mat_vector, 0.95)]
s = var(JI_mat_vector_rmtop) / mean(JI_mat_vector_rmtop)
a = mean(JI_mat_vector_rmtop)^2 / var(JI_mat_vector_rmtop)
gamma_p = (1-pgamma(as.vector(JI_mat), shape=a, scale=s))
gamma_p_fdr = p.adjust(gamma_p, 'fdr')
gamma_limit = min(as.vector(JI_mat)[gamma_p<0.05])
##################
dir.create('tf_JI_plots')
### plot hist JI
pdf('tf_JI_plots/tf.JI_mat.hist.pdf', width=14, height=14)
par(mfrow=c(2,2))
hist(JI_mat, breaks=50)
hist(log10(JI_mat+1e-4), breaks=50)
hist(JI_mat, breaks=50)
abline(v=gamma_limit, col='red')
dev.off()


### get JI distance
JI_dist = dist(1-(JI_mat))
hc = hclust(JI_dist, "complete")
###
dist_within = c()
nclustr = 50
for (i in c(1:nclustr)){
cluster_id = cutree(hc, i)
dist_within_i = 0
for (j in c(1:i)){
dist_within_j = sum(1-JI_mat[cluster_id==j, cluster_id==j])
dist_within_i = dist_within_i + dist_within_j
}
dist_within[i] = dist_within_i
}
### plot JI dist curve
pdf('tf_JI_plots/tf.JI_dist.curve.pdf')
par(mfrow=c(1,1))
plot(1:nclustr, dist_within, log='')
lines(1:nclustr, dist_within)
dev.off()

############
### cut tree
ncluster = 20
cluster_id = cutree(hc, ncluster)

### plot pheatmap
corlo_lim_scale_max = max(JI_mat+1e-4)
corlo_lim_scale_min = min(JI_mat+1e-4)
breaksList_scale = seq(corlo_lim_scale_min, corlo_lim_scale_max, by = 0.001)
my_colorbar_scale=colorRampPalette(c('white', 'blue'))(n = length(breaksList_scale))
JI_dist = dist(1-(JI_mat))
#JI_dist = dist(t(log10(JI_mat+1e-4)))
pdf('tf_JI_plots/tf.JI_mat.pheatmap.pdf', width = 30, height = 30)
pheatmap((JI_mat+1e-4), clustering_distance_cols=JI_dist, clustering_distance_rows=JI_dist, color=my_colorbar_scale, breaks = breaksList_scale, annotation_names_col = FALSE)
dev.off()

JI_mat_lim = JI_mat
JI_mat_lim[JI_mat<gamma_limit]=0
pdf('tf_JI_plots/tf.JI_mat.pheatmap.lim.pdf', width = 30, height = 30)
pheatmap((JI_mat_lim+1e-4), clustering_distance_cols=JI_dist, clustering_distance_rows=JI_dist, color=my_colorbar_scale, breaks = breaksList_scale, annotation_names_col = FALSE)
dev.off()

tf_files_names_exist_len = apply(cbind(tf_files_names_exist), 1, function(x) length(unlist(strsplit(x,''))))
tf_files_names_max = tf_files_names_exist[tf_files_names_exist_len==max(tf_files_names_exist_len)]
label_matrix = cbind(cluster_id, cluster_id)
colnames(label_matrix) = c(tf_files_names_max,tf_files_names_max)
cluster_colors = colorRampPalette(rev(brewer.pal(n = ncluster, name ="Paired")))(ncluster)
set.seed(2018)
cluster_colors = cluster_colors[sample(length(cluster_colors), length(cluster_colors))]
cluster_colors = c('#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#bcf60c', '#fabebe', '#008080', '#e6beff', '#9a6324', '#fffac8', '#800000', '#aaffc3', '#808000', '#ffd8b1', '#000075', '#808080')#, '#ffffff', '#000000')
pdf('tf_JI_plots/tf.JI_mat.pheatmap.cluster.pdf', width = 5, height = 30)
pheatmap(label_matrix, clustering_distance_rows=JI_dist, annotation_names_col = FALSE, color = cluster_colors)
dev.off()

for (i in 1:ncluster){
output_name = paste('tf_JI_plots/cluster_', i, '_list.txt', sep='')
file_list_tmp = rownames(label_matrix)[label_matrix[,1]==i]
write.table(file_list_tmp, output_name, sep='\t', quote=F, col.names=F, row.names=F)
}


##################
### get JI network
JI_mat_net = c()
for (i in c(1:dim(JI_mat)[1])){
for (j in c(1:dim(JI_mat)[1])){
if (i>j){
if (JI_mat[i,j]>gamma_limit){
#if (JI_mat[i,j]>0){
info_tmp = c(colnames(JI_mat)[i], colnames(JI_mat)[j], JI_mat[i,j], cluster_id[i], cluster_id[j], cluster_colors[cluster_id[i]], cluster_colors[cluster_id[j]])
JI_mat_net = rbind(JI_mat_net, info_tmp)
}
}
}
}

colnames(JI_mat_net) = c('s', 't', 'JI', 'sc', 'tc', 'scc', 'tcc')
write.table(JI_mat_net, 'tf_JI_plots/JI_mat_net.txt', col.names=T, row.names=F, sep='\t', quote=F)


'''
### log10 scale
corlo_lim_scale_max = max(log10(JI_mat+1e-4))
corlo_lim_scale_min = min(log10(JI_mat+1e-4))
breaksList_scale = seq(corlo_lim_scale_min, corlo_lim_scale_max, by = 0.001)
my_colorbar_scale=colorRampPalette(c('white', 'blue'))(n = length(breaksList_scale))

JI_dist = dist(t(log10(1-JI_mat)+0.001))
pdf('tf_JI_plots/tf.JI_mat.pheatmap.log10.pdf', width = 30, height = 30)
pheatmap(log10(JI_mat+1e-4), clustering_distance_cols=JI_dist, clustering_distance_rows=JI_dist, color=my_colorbar_scale, breaks = breaksList_scale, annotation_names_col = FALSE)
dev.off()
'''




...
parafile1='~/group/projects/vision/withMETH/run_IDEAS_result_lesshet/run_IDEAS_lesshet.para'
parafile2='~/group/projects/vision/pknorm_2_16lim_ref1mo_sample1mo_0424/pknorm_2_16lim_ref1mo_0424_lesshet.para'
statecolor = NULL
markcolor = NULL
show=TRUE
fout=NULL
...
scale=F
	...
	#x2_m = cbind(x2[,c(1:9)], rep(min(c(min(1), min(x2[,1:9]))), dim(x2)[1]));
	#x2_m = t(apply(x2[,c(1:9)], 1, function(x) c(x,min(x[-1]))));
	x2_m = t(apply(x2[,c(1:9)], 1, function(x) c(x,x[1]*1)));
	colnames(x2_m) = colnames(x1[,c(1:10)]);
	x = rbind(x1[,c(1:10)], x2_m)
	k=dim(x1)[2];
	l=dim(x)[1];
	l1=dim(x1)[1];
	l2=dim(x2)[1];
	...
	m0 = m[,-6]
	marks=colnames(m);
	rn1 = paste('M:',1:l1-1," (",round(x1[,1]/sum(x1[,1])*10000)/100,"%)",sep="");
	rn2 = paste('U:',1:l2-1," (",round(x2[,1]/sum(x2[,1])*10000)/100,"%)",sep="");
	#rownames(m)=paste(1:l-1," (",round(x[,1]/sum(x[,1])*10000)/100,"%)",sep="");
	rownames(m)=c(rn1, rn2)
if(sortstate)
{
o=hclust(dist(m0),method="complete")$order;
m=m[o,];

...
for (i in c(1: dim(signal_matrix)[2]) ){
	...
	for (j in c( 1: length(ideas_state_matrix_uniq_sort))[-c(10,14)] ){
	...
	### vector to matrix
	counts_index_matrix = rbind(counts_index_matrix, table_tmp[1])
...
pdf(paste(cREs_IDEASpro_outfile, '.reorder.not_0_9_13.pdf', sep=''), height=3.5,width=5)
bb = barplot(counts_matrix_t_sort, col=my_colorbar[-c(10,14)], ylim=c(0,barplot_lim),las=2)

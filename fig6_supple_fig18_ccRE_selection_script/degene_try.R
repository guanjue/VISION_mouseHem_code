...
DEreg<-function(chr, path, statepref, frna = "genematrix.txt", fhic = NULL, hicresolution = 1e4, usemean = F, maxdist=200000, fpref="step", direction="inc", resume=NULL, multi=FALSE)
{	tt = prepareGene(frna, chr);
...
tss = tt$tss;
	...
	{	print(i);
		...
		{	ee = getStateCoef(paste(path,statepref,sep=""), frna);
		} else
		{	ee = read.table(str);
		}
...
if(length(fhic) > 0)
{	if(file.exists(fhic))
	{	h=log2(as.matrix(fread(fhic))+1);
		h=gaussSmooth(h,1);
		h=(h+0.01)/max(h+0.01)/2;
	}
}
...
if(length(h)==0)
{	prior = exp(-abs(pos[rinv[j,1]:rinv[j,2]]-(pos[tinv[j,1]]+pos[tinv[j,2]])/2)/1000/200) / 3;
} else
{	prior = h[as.integer(pos[rinv[j,1]:rinv[j,2]]/hicresolution)*dim(h)[1]+as.integer((pos[tinv[j,1]]+pos[tinv[j,2]])/2/hicresolution)+1];
}
plot(prior,type="h");
			#X = smoothMatrix(X);
			yy = Y[gid[j],];
...
#			ii = ii[is.na(ii)==F];
			...
			{	rti = runStepMerge1(X[,a], yy, prior[a], penalty=mpenalty, direction = direction);
			}
...
stepMerge_inc<-function(x, y, prior, penalty, sel=NULL, sz = 0)
{	n = length(y);
	x = array(x, dim=c(n, length(x)/n));
	l = dim(x)[2];	
	#y = y - mean(y);
	...
	prior = log(prior + 1e-5) - log(1 - prior + 1e-5);
	...
	{	j = 1;
		nlp = apply(x,2,function(z)
			{	#dy = y-(XY+xy[j])/sum((X+z)^2+1e-5)*(X+z);
				#v = var(dy);
				tlp = getlp(X+z,y,XY+xy[j],sk+1,n) + prior[j];
				j <<- j+1;
...
stepMerge_dec<-function(x, y, prior, penalty, sel=NULL)
{	n = length(y);
	x = array(x, dim=c(n, length(x)/n));
	l = dim(x)[2];	
#	y = y - mean(y);
	...
	{	j = 1;
		nlp = apply(array(x[,sel],dim=c(n,length(sel))),2,function(z)
			{	#dy = y-(XY-xy[sel[j]])/sum((X-z)^2+1e-5)*(X-z);
				#v = var(dy);
				tlp = getlp(X-z,y,XY-xy[sel[j]],sk-1,n) - prior[sel[j]];
				j <<- j+1;
				#return((-sum(dy^2)/v-n*log(v))/2);
				return(tlp);
			});
...
DEpredict<-function(chr, path, statepref, frna = "genematrix.txt", fhic=NULL, hicresolution=1e4,maxdist = 200000, fpref = "step", direction="inc", gidlist = NULL)
{	tt = prepareGene(frna, chr);
#	gene=read.table("Ensembl_v65.Gencode_v10.ENSG.gene_info");
#	Y=array(0,dim=c(dim(gene)[1],56));
#	x=read.table("57epigenomes.RPKM.pc",header=T);
...
#	cellmap = match(cells, cellinfo[,1]);
	...
	pos = as.matrix(g[,3]);
	#state = as.matrix(g[,5:131])[,cellmap];
state = as.matrix(g[,-(1:4)]);
state = state[, match(colnames(Y),colnames(state))];
	t = which(gene[,2]==substr(i,4,1000));
...
{	if(file.exists(fhic))
	{	h=log2(as.matrix(fread(fhic))+1);
		#h=as.matrix(fread(fhic));
		for(i in 1:dim(h)[1]-1)
		{t=(1:(dim(h)[1]-i)-1)*dim(h)[1]+1:(dim(h)[1]-i)+i;h[t]=h[t]-mean(h[t]);
		 if(i>0)
		 { t=(1:(dim(h)[1]-i)+i-1)*dim(h)[1]+1:(dim(h)[1]-i);h[t]=h[t]-mean(h[t]);
		 }
		}
		h=log2(2^h+1);
		h=gaussSmooth(h,1);
		h=((h+0.01)/max(h+0.01))^1/2;
	}
}
	for(j in gidlist)
	{	
...
{	prior = h[as.integer(pos[rinv[j,1]:rinv[j,2]]/hicresolution)*dim(h)[1]+as.integer((pos[tinv[j,1]]+pos[tinv[j,2]])/2/hicresolution)+1];
#	tprior = log2(prior * 100 + 1);
#	tprior = tprior - mean(tprior)
#	tprior[tprior<0]= 0;
#	tprior = 2^tprior - 1;
#	tprior = tprior / sum(tprior) * sum(prior);
#	tprior = tprior / max(tprior) / 2;
#	tprior[tprior < 0.01] = 0.01;
#	prior = tprior;
}
plot(prior,type="h");
		#X = smoothMatrix(X);
		...
		{	ff[k] = mean(yy[-k]);
			...
			rti = runStepMerge1(X[-k,a], yy[-k], prior[a], mcmc=100, direction = direction,penalty=0.);#log(nn-1)/4);
			ii = a[rti$ss];
...
runStepMerge1<-function(x, y, prior, penalty = 1, B = 10, burnin = 100, mcmc = 400, direction = "inc")
{	l = length(x) / length(y);
	x = array(x,dim=c(length(y),l));
	sz = min(l, 40);
	nn = ceiling(l / sz);
	...
	{	a = (i-1)*sz + 1:sz;
		a = a[a<=l];
tss=stepMerge(x[,a],y,prior[a],burnin=1,mcmc=5);
a=a[which(tss>1)];
if(length(a)==0) next;
		#if(direction=="inc")
...
#	ss = ss[tt];
	...
	if(direction == "inc")
	{	ss = sort(ss[stepMerge_inc(x[,ss],y,prior[ss],penalty=penalty)]);
	} else
	{	ss = sort(ss[stepMerge_dec(x[,ss],y,prior[ss],penalty=penalty)]);#, sel=which(p[tt]>0.99))]);
	}
...
getStateCoef<-function(statepref, frna = "genematrix.txt")
{	tt = prepareGene(frna);
	gene = tt$gene;
	Y = tt$expr;
	tss = tt$tss;
	...
	ee = getStateCoef_core(Y, tss, genechr, as.matrix(gene[,6]), statepref);
	return(ee);
}

getStateCoef_core<-function(rna, tss, genechr, genetype, statepref, suffix="chr1.")
{	
	library("data.table");
	library("MASS");
	celln = dim(rna)[2];
	a=system(paste("wc -l ",statepref,suffix,"para",sep=""),intern=T);
	gn=as.integer(substr(a,1,regexpr(" ",a)[1]))-1;
	X = array(0, dim=c(dim(rna)[1]*celln,gn));
	sel = rep(1,dim(rna)[1]);
	for(i in unique(genechr))
	{	print(i);
		Z = NULL;
		if(suffix=="chr1.")
		{	g=fread(paste(statepref,i,".state",sep=""));
		} else
		{	g=fread(paste(statepref,"state",sep=""));
		}
		tchr = as.matrix(g[,2]);
		tt = which(i == tchr);
		pos = as.matrix(g[,3])[tt];
		state = as.matrix(g[,-(1:4)])[tt,];
	...
	geneid = unique(genetype);
	ee = array(0, dim=c(length(geneid),gn));
	for(i in 1:length(geneid))
	{	t = which(genetype == geneid[i] & sel == 1);
		if(length(t)>0)
		{	ty = c(t(rna[t,]));
...
gaussSmooth<-function(X,w=3,maxdist=250)
{	if(w==0) return(X);
	l=dim(X)[1];
	w2=(w+1)*2;
	if(w2>0)
	...
	d=dnorm(-w2:w2,0,max(w,0.5));
	D=(d%*%t(d));
	nX=array(0,dim=rep(l,2));
	
	maxdist=min(l-1,maxdist);
	for(i in w2+1:l)
	{	tt = i+-w2:(maxdist+w2);
		tt = tt[tt<=dim(X)[1]];
		ll = length(tt) - 2 * w2 - 1;
		...
		td=tx%*%D;
		for(j in 1:(2*w2+1))
			nX[i-w2+0:ll,i-w2]=nX[i-w2+0:ll,i-w2]+td[0:ll+j,j];
		nX[i-w2,i-w2+0:ll]=nX[i-w2+0:ll,i-w2];
		if(i%%1000==0) cat(i,"");
	}
	print("done");
	return(nX);
...
prepareGene<-function(frna = "genematrix.txt", chr = NULL)
{
if(FALSE)
{
	gene=read.table("Ensembl_v65.Gencode_v10.ENSG.gene_info");
...
x = read.table(frna,header=T);
if(length(chr)>0)
{	x = x[paste("chr",as.matrix(x[,2]),sep="") == chr,];
}
Y = as.matrix(x[,-(1:8)]);
gene = x[,1:8];
tss = x[,3];
...
maketracks<-function(fcrespref, fstatepref, chrs, fout, cut=0.2)
{	
	library("data.table");
	ll = NULL;
	for(r in 1:length(chrs))
	{	print(r);
		x=readLines(paste(fcrespref,chrs[r],".predict_crelist",sep=""));
		x=x[regexpr("NA",x)<0];
		a=t(array(unlist(strsplit(x," ")),dim=c(4,length(x))));	
		a=a[as.numeric(a[,2])>cut,];
		g=fread(paste(fstatepref,chrs[r],".state",sep=""));
		pos = as.matrix(g[,3]);
		l=length(pos);
		n=rep(0,l);
		for(i in 1:dim(a)[1])
		{	t = unlist(strsplit(a[i,4],","));
			k = as.integer(regexpr(":",t));
			p = as.integer(substr(t,1,k-1));
			m = as.integer(substr(t,k+1,1000));
			n[p] = n[p] + m;	
		}
		pos = pos[-length(pos)];
		n = n[-length(n)];
		t = which(n>0);
		if(length(t) > 0) ll = c(ll, paste(chrs[r],pos[t],pos[t]+200,n[t],sep=" "));
	}
	str="track type=bedGraph name=\"BedGraph Format\" description=\"BedGraph format\" visibility=full color=200,100,0 altColor=0,100,200 priority=20";
	options(scipen=1000);
	write.table(c(str,ll),fout,quote=F,row.names=F,col.names=F);
}

calMin1<-function(x,cut=0,hsz=50)
{	tt=which(is.na(x)==F);
	l0=length(x);
	x=x[tt];
	l=length(x);
	if(l < 10) return(rep(0,l0));
	if(cut==0) cut=sd(x[-1]-x[-length(x)])/sqrt(hsz)*3.89;

	a=rep(0,l);
	for(i in 1:l)
	{	t=max(1,i-hsz):min(l,i+hsz);
		a[i]=as.integer(x[i]==min(x[t]) & max(x[t])-min(x[t]) > cut);
	}
	aa=rep(0,length(x));
	aa[tt]=a;
	return(aa);
